...
!developmentChains.includes(network.name)
    ...
    : describe("Raffle Unit Tests", function () {
          ...
          describe("enter-raffle", function () {
              it("it reverse when they dont pay enough", async function () {
              ...
              it("it emits events on enter", async function () {
          ...
          describe("performUpkeep", function () {
              ...
              it("updats the raffle state, emits and event , and calls the vrf coordinator", async function () {
                  await raffle.enterRaffle({ value: raffleEntranceFee })
                  await network.provider.send("evm_increaseTime", [interval.toNumber() + 1])
                  await network.provider.send("evm_mine", [])
                  const txResponse = await raffle.performUpkeep([])
                  const txReceitpt = await txResponse.wait(1)
                  const requestId = txReceitpt.events[1].args.requestId
                  const raffleState = await raffle.getRaffleState()
